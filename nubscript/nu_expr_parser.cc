...
expr_any_t::handle_t expr_parser_t::parse_operand(token_list_t& tl)
{
    ...
    case tkncl_t::OPERATOR:
        ...
        if (tl.size() > 1 && (tl[0].identifier() == NUBSCRIPT_OP_INC
                                 || tl[0].identifier() == NUBSCRIPT_OP_DEC)
        ...
        if (tl.size() > 1 && tl[1].type() == tkncl_t::SUBEXP_BEGIN) {
            ...
            if (tl.size() > 2 && tl[2].type() == tkncl_t::SUBEXP_END) {
                ...
                auto params = tl.get_parameters(
                    { NUBSCRIPT_BEGIN_SUBEXPR_OP, tkncl_t::SUBEXP_BEGIN },
                    { NUBSCRIPT_END_SUBEXPR_OP, tkncl_t::SUBEXP_END },
                    { NUBSCRIPT_PARAM_SEP, tkncl_t::OPERATOR });
            ...
            if (function_name.size() > 1
                && *function_name.rbegin() == NUBSCRIPT_BEGIN_SUBSCR) {
                ret_handle = std::make_shared<expr_subscrop_t>(
            ...
            // 0xnnnnnn  (hexadecimal value)
                ...
                if (id.size() > 1 && *id.rbegin() == NUBSCRIPT_BEGIN_SUBSCR)
                ...
                ret_handle
                    = expr_any_t::handle_t(std::make_shared<expr_var_t>(id));
            }
...
expr_any_t::handle_t expr_parser_t::parse(
    token_list_t& tl, expr_any_t::handle_t first_param)
{
    fix_real_numbers(tl);
    ...
    syntax_error_if(err, NUBSCRIPT_ERROR_STR__SYNTAXERROR);

    second_param = parse_operand(tl);
    syntax_error_if(!second_param, NUBSCRIPT_ERROR_STR__SYNTAXERROR);
    ...
    return expr_any_t::handle_t(std::make_shared<expr_bin_t>(
        operator_implementation, first_param, second_param));
}
...
void expr_parser_t::fix_real_numbers(token_list_t& rtl)
{
    ...
    for (decltype(tl_size) i = 0; i < tl_size; ++i) {
        ...
        if (tk.type() == tkncl_t::REAL) {
            ...
            if (!id.empty()
                && ::toupper(id.c_str()[id.size() - 1])
                    == NUBSCRIPT_EXPONENT_SYMB
                && i < (tl_size - 1)) {
                ...
                if (tkop.type() == tkncl_t::OPERATOR
                    ...
                    if (tkn.type() == tkncl_t::INTEGRAL) {
                        std::string new_id = tk.identifier() + tkop.identifier()
                            + tkn.identifier();
...
{
    // simplify "(((....)))"
    while (rtl.size() > 2 && rtl.begin()->type() == tkncl_t::SUBEXP_BEGIN
        && (rtl.end() - 1)->type() == tkncl_t::SUBEXP_END) {
        auto substr
            = rtl.sublist(tkncl_t::SUBEXP_BEGIN, tkncl_t::SUBEXP_END, 0, false);
    ...
    for (size_t i = 0; i < rtl.size(); ++i) {
        ...
        if (token.type() == tkncl_t::SUBSCR_BEGIN) {
            if (token_prev) {
                ...
                if (!id.empty() && *id.rbegin() != NUBSCRIPT_BEGIN_SUBSCR) {
                    id.push_back(NUBSCRIPT_BEGIN_SUBSCR);
            ...
            if (token.identifier() == NUBSCRIPT_BEGIN_SUBSCR_OP)
                token.set_identifier(
                    NUBSCRIPT_BEGIN_SUBEXPR_OP, nu::token_t::case_t::LOWER);
        }

        else if (token.type() == tkncl_t::SUBSCR_END) {
            token.set_type(tkncl_t::SUBEXP_END);

            if (token.identifier() == NUBSCRIPT_END_SUBSCR_OP)
                token.set_identifier(
                    NUBSCRIPT_END_SUBEXPR_OP, nu::token_t::case_t::LOWER);
        }
    }
}
