...
std::list<token_list_t> token_list_t::get_parameters(btfunc_t test_begin,
...
token_list_t token_list_t::sublist(
    ...
    std::function<bool(const token_t&)> test_end, size_t& search_from,
    bool b_erase)
{
    assert(search_from < size());
...
token_list_t token_list_t::sublist(const token_t& first, const token_t& second,
