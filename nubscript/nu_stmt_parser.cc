...
void stmt_parser_t::extract_next_token(token_list_t& tl, token_t& token,
    ...
    prog_ctx_t& ctx, token_t token, nu::token_list_t& tl)
{
    ...
    if (tl.empty()) {
        return stmt_t::handle_t(std::make_shared<stmt_print_t>(ctx, 0));
    }
    ...
    if (token.type() == tkncl_t::STRING_LITERAL && tl.size() == 1) {
        ...
        return stmt_t::handle_t(
            std::make_shared<stmt_print_t>(ctx, 0, token.identifier()));
    }
...
stmt_t::handle_t stmt_parser_t::parse_write(
    prog_ctx_t& ctx, token_t token, nu::token_list_t& tl)
{
    --tl;
    remove_blank(tl);
...
void stmt_parser_t::parse_fd_args(prog_ctx_t& ctx, token_t token,
...
stmt_t::handle_t stmt_parser_t::parse_block(prog_ctx_t& ctx,
...
stmt_t::handle_t stmt_parser_t::parse_let(prog_ctx_t& ctx, nu::token_list_t& tl)
{
    remove_blank(tl);
    ...
    prog_ctx_t& ctx, token_t token, nu::token_list_t& tl)
{
    ...
    move_sub_expression(tl, // source token list
    ...
    prog_ctx_t& ctx, token_t token, nu::token_list_t& tl)
{
    ...
    if (tl.size() > 1 && (tl.begin() + 1)->type() == tkncl_t::OPERATOR
        ...
        return tl.empty() ? std::make_shared<stmt_empty_t>(ctx)
                          : parse_block(ctx, tl);
    }
    ...
    prog_ctx_t& ctx, nu::token_list_t& tl)
{
    ...
    syntax_error_if((token.type() != tkncl_t::IDENTIFIER)
            && (token.type() == tkncl_t::OPERATOR
                        && (token.identifier() != NUBSCRIPT_OP_INC
                               && (token.identifier() != NUBSCRIPT_OP_DEC))),
        token.expression(), token.position());

    if (identifier == NUBSCRIPT_OP_INC || identifier == NUBSCRIPT_OP_DEC)
    ...
    if (identifier == "write")
        return parse_write(ctx, token, tl);
    ...
    nu::token_list_t& tl, prog_ctx_t& ctx)
{
    ...
    if (!tl.empty()) {
        ...
        case tkncl_t::OPERATOR:
            syntax_error_if(token.identifier() != NUBSCRIPT_OP_INC
                    && token.identifier() != NUBSCRIPT_OP_INC,
                token.expression(), token.position());
            break;
